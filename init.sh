...
appSetup () {
	...
	if [[ ! -f /etc/samba/external/smb.conf ]]; then
		...
		if [[ ${JOIN,,} == "true" ]]; then
			...
			samba-tool domain provision --use-rfc2307 --domain=${URDOMAIN} --realm=${UDOMAIN} --server-role=dc --dns-backend=SAMBA_INTERNAL --adminpass=${DOMAINPASS} --option="ad dc functional level = 2016" --function-level=2016 ${HOSTIP_OPTION}
